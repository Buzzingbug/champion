...
# Heavy blur for pure, smooth atmospheric lighting
...
# 2. Main Outer Translucent Glass Container (DO NOT block the light! fill alpha=45 only!)
outer_glass = Image.new('RGBA', (width, height), (0, 0, 0, 0))
og_draw = ImageDraw.Draw(outer_glass)
outer_box = [(25, 25), (width - 25, height - 25)]
# High transparency dark glass so the backlight shines through cleanly!
...
# 3. Inner Glass Panes (Translucent glass cards, letting backlights flood through!)
glass_panes = Image.new('RGBA', (width, height), (0, 0, 0, 0))
gp_draw = ImageDraw.Draw(glass_panes)

# Helper for true translucent glass cards
...
# 4 Mid Row Tiles (Roomy, perfectly translucent!)
...
# Bottom 4 Bento Glass Bases (Light, translucent so colored floods illuminate them!)
...
# 4. NEON FRONT GLOW (Drawn ON TOP of the cards so pink/gold/cyan glows directly in the front!)
front_neon = Image.new('RGBA', (width, height), (0, 0, 0, 0))
fn_draw = ImageDraw.Draw(front_neon)

# Multi-layer neon bloom for Avatar Halo
fn_draw.ellipse((av_center[0] - av_r, av_center[1] - av_r, av_center[0] + av_r, av_center[1] + av_r), outline=(255, 170, 0, 255), width=8)

# Multi-layer neon bloom for Bottom 4 Cards
fn_draw.rounded_rectangle([(40, 725), (375, 945)], radius=24, outline=(255, 160, 0, 255), width=5)
fn_draw.rounded_rectangle([(395, 725), (730, 945)], radius=24, outline=(245, 45, 210, 255), width=6) # HOT PINK GLOW ON FRONT!
...
# Gaussian blur for radiant bloom
...
tl_draw.rounded_rectangle([(395, 725), (730, 945)], radius=24, outline=(255, 110, 235, 255), width=2) # CRISP PINK CORE!
...
# --- Themed Illustrations in the Cards ---
# A. Messages: Chat Bubbles
...
# 4 Mid Row Tiles (Clean, unclipped, plenty of room!)
# Tile 1: Server Rank
...
img.convert("RGB").save("scratch/neon_mockup_v4.png")
print("Saved scratch/neon_mockup_v4.png successfully!")
