...
class SmashOrPassView(discord.ui.View):
    ...
    async def process_vote(self, interaction: discord.Interaction, choice: str):
        config = self.bot.cache['smash'].get(interaction.guild.id)
        if not config:
            await interaction.response.send_message("Smash or Pass is not configured for this server.", ephemeral=True)
            return
    ...
    @discord.ui.button(label="Smash", style=discord.ButtonStyle.success, emoji="🟢", custom_id="smashorpass_smash")
    ...
    @discord.ui.button(label="Pass", style=discord.ButtonStyle.danger, emoji="🔴", custom_id="smashorpass_pass")
...
class SmashOrPassCog(commands.Cog):
    ...
    async def setup(self, interaction: discord.Interaction, channel: discord.TextChannel, role: discord.Role, reward: int, message: str, post_cost: int = 0):
        ...
        async with self.bot.db_pool.acquire() as connection:
            ...
            await connection.execute(
                """
                INSERT INTO server_configs (guild_id, smash_channel_id, smash_role_id, smash_reward_amount, smash_custom_message, post_cost)
                VALUES ($1, $2, $3, $4, $5, $6)
                ON CONFLICT (guild_id) DO UPDATE 
                SET smash_channel_id = $2, smash_role_id = $3, smash_reward_amount = $4, smash_custom_message = $5, post_cost = $6
                """,
                interaction.guild.id, channel.id, role.id, reward, message, post_cost
            )
            
        self.bot.cache['smash'][interaction.guild.id] = {
            'guild_id': interaction.guild.id,
            'smash_channel_id': channel.id,
            'smash_role_id': role.id,
            'smash_reward_amount': reward,
            'smash_custom_message': message,
            'post_cost': post_cost
        }
    ...
    async def submit(self, interaction: discord.Interaction, image: discord.Attachment):
        ...
        if interaction.channel.id != config['smash_channel_id']:
            await interaction.response.send_message(f"You can only submit images in <#{config['smash_channel_id']}>.", ephemeral=True)
            return
    ...
    async def on_message(self, message: discord.Message):
        ...
        config = self.bot.cache['smash'].get(message.guild.id)
        if not config or message.channel.id != config['smash_channel_id']:
            return
